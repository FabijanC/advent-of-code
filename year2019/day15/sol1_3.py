...
while True:
    ...
    if command == HALT:
        ...
        if status == 0:
            ...
            print("wall, reverting")
            revert(storage)
            continue
        elif status == 2:
            area[pos_y, pos_x] = FOUND, length
            break
        ...
        for movement_code, delta_y, delta_x in movements:
            ...
            if other_kind != WALL and length < other_length:
                break
        else:
            print("no eligible adjacent, reverting")
            revert(storage)
            continue

        write(i+1, modes[-1], movement_code)
        stack.append((new_pos[0], new_pos[1], length+1, store()))
        i += 2
    elif command == PUT:
